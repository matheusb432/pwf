...
bench benchmark="all" case="" update="--baseline local" quick="":
    CRITERION_HOME="{{ justfile_directory() }}/.artifacts/benchmarks/criterion" cargo bench --locked {{ if benchmark == "all" { "--workspace --benches" } else if benchmark == "pwf-marker-sections" { "-p pwf-marker-sections --bench pwf_marker_sections" } else if benchmark == "list-tasks-rpc" { "-p pwf-server --bench list_tasks_rpc" } else { "-p pwf-infra --bench " + replace(benchmark, "-", "_") } }} -- {{ if case == "" { "" } else { quote(case) + " --exact" } }} {{ update }} {{ quick }}
