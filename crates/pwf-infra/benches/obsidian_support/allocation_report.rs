...
use super::fixture::{FixtureManifest, benchmark_artifact_root, manifest};
...
pub fn run(benchmark: &str, cases: Vec<AllocationCase>) -> Result<()> {
    let update = parse_arguments()?;
    let compatibility = compatibility(benchmark, &cases)?;
    let report_directory = benchmark_artifact_root()
        .join("allocations")
        .join(benchmark);
    ...
    for case in cases {
        let samples = (0..manifest().allocation_sample_count)
...
fn compatibility(benchmark: &str, cases: &[AllocationCase]) -> Result<Compatibility> {
    Ok(Compatibility {
        schema_version: REPORT_SCHEMA_VERSION,
        benchmark: benchmark.to_string(),
        fixture: manifest().clone(),
        allocation_sample_count: manifest().allocation_sample_count,
