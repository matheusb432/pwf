...
use pwf_models::{
    settings::{RgbColor, TaskStatusColors, UserSettings},
    task::{
        PriorityTier,
...
mod tests {
    ...
    fn valid_response() -> pb::GetUserSettingsResponse {
        pb::GetUserSettingsResponse {
            task_status_colors: Some(pb::TaskStatusColors::default()),
    ...
    fn settings_response_validates_list_defaults() {
        assert_eq!(
            decode(valid_response()).unwrap(),
            pwf_models::settings::UserSettings::default()
        );
