...
use pwf_application::pending_work::{
    complete_pending_work::CompletePendingWorkOk, reopen_pending_work::ReopenPendingWorkOk,
};

use super::confirmation::render_review_item;

/// Renders the byte-stable close confirmation and optional review-task allocation.
pub(in crate::pending_work) fn render_closed(outcome: &CompletePendingWorkOk) -> String {
    let mut text = format!(
        "{} {} ({} :: {})\n",
...
pub(in crate::pending_work) fn render_reopened(outcome: &ReopenPendingWorkOk) -> String {
...
pub(in crate::pending_work) fn emit_close_diagnostics(outcome: &CompletePendingWorkOk) {
