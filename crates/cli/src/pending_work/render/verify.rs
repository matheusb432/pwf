...
use pwf_application::pending_work::session::VerifySessionOk;
use pwf_infra::session::render_argv;

pub(in crate::pending_work) fn render_verify(outcome: &VerifySessionOk) -> String {
    ...
    let _ = writeln!(out, "command: {}", render_argv(&outcome.command_argv));
...
#[cfg(test)]
mod tests {
    use pwf_application::pending_work::session::AgentProbe;
    ...
    fn outcome(binary: &str, available: bool, launchable: bool) -> VerifySessionOk {
        VerifySessionOk {
            ...
            command_argv: vec![
                binary.to_string(),
                "--".to_string(),
                "Pending-work ID: PWF-0001…".to_string(),
            ],
        }
    }
    ...
    fn verify_text_no_item_pass_when_agent_present() {
        ...
        result.command_argv = vec!["claude".to_string()];
        let out = render_verify(&result);
