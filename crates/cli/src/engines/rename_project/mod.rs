//! Relocates a managed project's notes directory, ID namespace, references, labels, and
//! `repos.toml` entry.
//!
//! This facade resolves inputs, checks collisions, and dispatches dry-run rendering or [`apply`].
...
/// Reports failures while resolving or applying a project rename.
...
/// Contains the resolved config and manifest inputs for a project rename.
...
/// Resolves and validates a [`RenameContext`] from the pwf config and `repos.toml`.
/// The vault root is derived from [`crate::config::Config::notes_dir_for`].
pub fn resolve(args: &EngineArgs) -> Result<RenameContext, RenameProjectError> {
    let old_code = args
        .old_code
        .clone()
...
/// Runs a project rename and converts typed failures to CLI strings.
pub fn run(args: &EngineArgs) -> Result<String, String> {
    run_typed(args).map_err(|e| e.to_string())
}

/// Resolves, preflights, and either renders or applies a project rename.
pub fn run_typed(args: &EngineArgs) -> Result<String, RenameProjectError> {
