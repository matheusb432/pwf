...
pub(in crate::engines::pending_work) fn render_added(item: &AddedItem, color_on: bool) -> String {
    render_confirmation(
        "Added pwf task",
        AnsiColor::Green,
        &item.id,
...
pub(in crate::engines::pending_work) fn render_removed(
    item: &RemovedItem,
    color_on: bool,
) -> String {
    render_confirmation(
...
pub(super) fn render_review_item(item: &AddedItem) -> String {
    format!(
        "ADDED PWF TASK [{}] {}\n  file: {}\n",
        item.id,
        added_headline(item),
...
fn added_headline(item: &AddedItem) -> String {
    format!("{} :: {}", item.project, item.title)
}
