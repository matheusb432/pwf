...
fn lookup_id(args: &EngineArgs) -> Result<&str, PendingWorkError> {
    let canonical = require_id(args, "show")?;
...
/// Returns the note's full Markdown, or its path when `markdown` is false.
...
where
    S: AppDbStore<PendingWorkItem> + NoteMarkdownSource,
{
    show_output(cfg, store, args, !args.path)
}
...
mod tests {
    ...
    fn show_of_missing_note_wikilink_errors_like_the_legacy_read() {
        ...
        let error = show_output(&cfg, &store, &id_args("PWF-0001"), true).unwrap_err();
    ...
    #[test]
    fn show_path_of_missing_note_wikilink_still_prints_expected_path() {
        let (_stage, cfg) = ghost_stage();
        let store = store_for(&cfg);

        let path = show_output(&cfg, &store, &id_args("PWF-0001"), false).unwrap();
