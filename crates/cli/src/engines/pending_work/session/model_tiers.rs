...
/// Errors loading or resolving `config/model-tiers.toml`. Crate-visible (not
/// wider): nested inside `PendingWorkError::ModelTiers`, which is itself
/// `pub(crate)` for the shared pending-work command-build seam — a `#[source]`
/// field can't be less visible than the enum wrapping it.
#[derive(Debug, Error)]
pub(crate) enum ModelTiersError {
