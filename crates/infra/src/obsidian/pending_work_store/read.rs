...
use super::{
    ...
    fs::{path_str, read_text_optional, read_text_or_default},
};
...
impl PendingWorkReadStore for ObsidianPendingWorkStore {
    ...
    fn open_items(&self, only_project: Option<&str>) -> Result<Vec<OpenItem>, Self::Error> {
        ...
        for project in project_names {
            ...
            items.extend(get_project_tasks(&project, repo, &index_path));
        }
...
fn get_project_tasks(project: &str, repo: Option<&str>, index_path: &Path) -> Vec<OpenItem> {
...
fn parse_file_model_items(
    ...
    for captures in LINK_RE.captures_iter(text) {
        ...
        items.push(OpenItem {
            ...
            needs_prompt: is_placeholder_prompt(&prompt),
            issues,
    ...
    legacy_matches
        ...
        .map(|(index, item)| {
            ...
            OpenItem {
                ...
                launchable: issues.is_empty(),
                needs_prompt: is_placeholder_prompt(&item.prompt),
                issues,
