...
{
    let document =
        <S as AppDbStore<HandoffDocument>>::insert(store, &pending.scope, pending.document)
    ...
    S: HandoffDocumentStore + AppDbStore<HandoffLedger>,
{
    ...
    <S as AppDbStore<HandoffDocument>>::delete(store, &pending.scope, &pending.snapshot.identifier)
