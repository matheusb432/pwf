...
use pwf_application::{
    note::{remove_note, remove_note::RemoveNoteError},
...
use crate::support::{InMemoryStore, InMemoryStoreFailure, insert_project, project_note};
...
async fn decline_preserves_the_note(pool: sqlx::SqlitePool) {
    insert_project(&pool, "FOO", "foo", "/projects/foo", "/tasks/foo", false).await;
    let store =
        InMemoryStore::default().with_project_notes("foo", vec![project_note(7, "remember milk")]);
