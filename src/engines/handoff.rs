...
fn spawn_pw_reopen(script: &str, cfg: &str, pw: &str) -> Result<(), HandoffError> {
    std::process::Command::new(script)
...
fn inprocess_pw_reopen(args: &Args, pw: &str) -> Result<(), HandoffError> {
    let a = crate::cli::Args {
...
/// [`find_handoff_file`], which only sees active handoffs.
...
fn reopen_handoff(root: &Path, args: &Args) -> Result<String, HandoffError> {
    let id = args.id.as_deref().ok_or_else(|| HandoffError::MissingId {
        action: args.action.clone().unwrap_or_default(),
    })?;
    let paths = handoff_paths(root);
