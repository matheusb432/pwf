...
mod tests {
    ...
    #[test]
    fn validated_types_serialize_as_strings_and_deserialize_through_validation() {
        let id = WorkItemId::try_new("PWF-0047").unwrap();
        assert_eq!(serde_json::to_string(&id).unwrap(), "\"PWF-0047\"");
        assert!(serde_json::from_str::<WorkItemId>("\"bad\"").is_err());
    }
}
